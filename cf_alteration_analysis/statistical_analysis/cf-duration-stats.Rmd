...
Let's build a LMM with the annotation-ID (recording ID) as the random intercept, and
the group size (single/multi) as a fixed effect. There can be multiple CF duration measurements 
from the same recording, and so this is the main source of correlation in the dataset.

```{r lmm-model}
lmm0 <- lmer(invdurn~as.factor(multi_bat)+(1|annot_id), data=d_tordered)
...
The final results do not seem to support a decrease in cF duration in groups. 

#### Model with only group size as fixed effect, and annotation id as random intercept

| Parameter     | 93% compatibility interval |
|---------------|----------------------------|
| CF duration, single (ms)| `r single.cfdurn.hpd[1]`-`r single.cfdurn.hpd[2]`|
| CF duration, multi (ms)| `r multi.cfdurn.hpd[1]`-`r multi.cfdurn.hpd[2]`|
| Relative CF duration increase ($\frac{multi}{single}$)| `r cfdurn.rel.hpd[1]`-`r cfdurn.rel.hpd[2]`|

The CF durations actually seem to overlap between single and multi bat calls. This is in contrast to the result we see from the individual call analysis. 
...
| Parameter     | 93% compatibility interval |
|---------------|----------------------------|
| CF duration, single (ms)| `r single.cfdurn.hpd.post[1]`-`r single.cfdurn.hpd.post[2]`|
| CF duration, multi (ms)| `r multi.cfdurn.hpd.post[1]`-`r multi.cfdurn.hpd.post[2]`|
| Relative CF duration increase ($\frac{multi}{single}$)| `r cfdurn.rel.hpd.post[1]`-`r cfdurn.rel.hpd.post[2]`|

```{r write-results}
df <- rbind(single.cfdurn.hpd, multi.cfdurn.hpd, cfdurn.rel.hpd)
row.names(df) <- c('single-cf-durn','multi-cf-durn','cf-durn-relincrease')
