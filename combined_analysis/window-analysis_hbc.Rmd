...
```{r domfreq}
obsdomf.m1 <- lmer(domfreq ~ group_status + centred_posix_minute+(1|video_annot_id), data=obs_domfreq)
obsdomf.m2 <- lmer(domfreq ~ group_status + (1|video_annot_id), data=obs_domfreq)

raneffects.m1 <- ranef(obsdomf.m1)$video_annot_id$'(Intercept)'
raneffects.m2 <- ranef(obsdomf.m2)$video_annot_id$'(Intercept)'

par(mfrow=c(2,2))
scatter.smooth(obs_domfreq$centred_posix_minute,resid(obsdomf.m1))
scatter.smooth(obs_domfreq$group_status,resid(obsdomf.m1))
qqnorm(resid(obsdomf.m1));qqline(resid(obsdomf.m1))
qqnorm(raneffects.m1);qqline(raneffects.m1)

frame()
par(mfrow=c(2,2))
scatter.smooth(obs_domfreq$centred_posix_minute,resid(obsdomf.m2))
scatter.smooth(obs_domfreq$group_status,resid(obsdomf.m2))
qqnorm(resid(obsdomf.m2));qqline(resid(obsdomf.m2))
qqnorm(raneffects.m2);qqline(raneffects.m2)

```
Both model regressions seem to broadly meet the normality assumptions. The prediction residuals are a bit off, but despite trying my hands at a bunch of transformations on the outcome variable, I've not had any luck in improving the residual fit. I will repeat the analysis with data points that don't have very large residuals, and see if the parameters are different. Here too I will move ahead with Model 2 (without time as an explanatory variable), as it has a lower AIC (Model 2:`r round(AIC(obsdomf.m2),0)` vs `r round(AIC(obsdomf.m1),0)`),  makes biological sense, and is easily interpretable. 
...
# remember to square the predictions to get the units back to dB from sqrt(dB)
single.domf <- fixef(obsdomf.m2.sim)[,1]
multi.domf <- fixef(obsdomf.m2.sim)[,1]+fixef(obsdomf.m2.sim)[,2]
