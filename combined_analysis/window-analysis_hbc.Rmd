...
```{r cleaning-and-subsetting, echo=T, warning=F,message=F,results='hide'}
...
obs_splitmeasure_timestamp['rows_to_remove'] <- lapply(obs_splitmeasure_timestamp$video_annot_id, annots.to.remove.present)

# check to see if all the rows are valid after the filtering
all_valid_rows <- all(!obs_splitmeasure_timestamp$rows_to_remove)
