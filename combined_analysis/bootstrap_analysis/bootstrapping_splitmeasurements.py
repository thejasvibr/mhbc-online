...
import numpy as np
try:
    import shuffle_overlap.shuffle_overlap as so
except:
    import shuffle_overlap as so
...
def calculate_bootstrapped_median_across_grouptypes(df, measurement_name, 
    ...
    df : pd.DataFrame
        ...
        file_name, segment_number, measurement_name, value
    measurement_name: str
        The type of measurement to be chosen
    summary_fn : function, optional 
        The function which calculates a summary value from the multiple 
        values that are in one segment's  measurement values. Defaults
        to just_return_input, a function which does not summarise at all.
    proc_fn : function, optional 
        Processing function, when raw values need to be scaled in some way,
        eg. dB. Converts all values into the scaled units. Defaults to 
        just_return_input - a function which does not alter the input values
        at all. 
    
    Returns
    -------
    median_data : list
        List with the median data across group types for one round of bootstrapping. 
    ...
    group123_measures = calc_group_type_summary(one_segment, 
                                                          measurement_name, 
                                                      summary_fn, proc_fn)
    median_data = [ np.nanmedian(each) for each in group123_measures]
    
    return median_data


def calculate_deltamedian_across_grouptypes(df, measurement_name, 
                                                    summary_fn=just_return_input,
                                                    proc_fn=just_return_input):
    '''
    Wrapper function around calculate_bootstrapped_median_across_grouptypes,
    which calculates the median across the group types after choosing 
    one segment per audio file. 
    
    This function uses the median values from each group to calculate 
    the difference in median values across groups. 
    ...
    See Also
    --------
    calculate_bootstrapped_median_across_grouptypes
    
    '''
    median_data = calculate_bootstrapped_median_across_grouptypes(df, measurement_name, 
                                                                      summary_fn,
                                                                      proc_fn)
    delta12 = median_data[0]-median_data[1]
    delta13 = median_data[1]-median_data[2]
    delta23 = median_data[1]-median_data[2]
    
    delta_median = [delta12, delta13, delta23]
    return delta_median
...
def bootstrapped_observed_overlap(df, measurement, **kwargs):
    ...
    single_multi,_ = so.calculate_overlap(one_segment_per_file[0]['value'],
                                                     one_segment_per_file[1]['value'], **kwargs)
    # overlap multi-virtualmulti
    multi_virtualmulti,_ = so.calculate_overlap(one_segment_per_file[1]['value'], 
                                                           one_segment_per_file[2]['value'], **kwargs)
    # overlap single-virtualmulti
    single_virtualmulti,_ = so.calculate_overlap(one_segment_per_file[0]['value'], 
                                                           one_segment_per_file[2]['value'], **kwargs)
...
def bootstrapped_shuffled_overlap(df, measurement, **kwargs):
    ...
    shuffled_single, shuffled_multi = shuffle_values_between_grouptypes(one_segment_per_file[0],
                                                                       one_segment_per_file[1],
                                                                       measurement)
    # overlap single-multi
    shuf_single_multi,_ = so.calculate_overlap(shuffled_single['value'],
                                                     shuffled_multi['value'], **kwargs)

    # -----------------------------------------------------------------------------------------
    # shuffle multi-virtualmulti
    shuffled_multi, shuffled_virtualmulti = shuffle_values_between_grouptypes(one_segment_per_file[1],
                                                                       one_segment_per_file[2],
                                                                       measurement)
    # overlap multi-virtualmulti
    shuf_multi_virtualmulti,_ = so.calculate_overlap(shuffled_multi['value'], 
                                                             shuffled_virtualmulti['value'], **kwargs)
    # -----------------------------------------------------------------------------------------
    
    # shuffle single-virtualmulti
    shuffled_single, shuffled_virtualmulti = shuffle_values_between_grouptypes(one_segment_per_file[0],
                                                                       one_segment_per_file[2],
                                                                       measurement)
    
    # overlap single-virtualmulti
    shuf_single_virtualmulti,_ = so.calculate_overlap(shuffled_single['value'], 
                                                           shuffled_virtualmulti['value'], **kwargs)
