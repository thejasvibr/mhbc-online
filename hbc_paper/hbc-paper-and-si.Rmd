...
1. Clean up table for ind. call results: change column names for lower and upper CI or fuse them into one column
1. Change results section 
...
We ran linear mixed-models (LMMs) with group size as the predictor and recording hour as random intercept for all parameters. For CF peak frequencies, we calculated a derived parameter, the range difference ($Range_{multi}-Range_{single}$) of CF peak frequencies across all multi bat and single bat calls. The results from the LMMs allowed us to estimate the means of single and multi bat calls, along with the difference in the means. Model coefficients were estimated using a Bayesian approach with non-informative priors, and the 93% compatibility intervals were used in reporting.
...
```{r incalldata, echo=FALSE, tab.cap="\\label{tab:incalldata} *Difference between multi and single bat call parameters. The 93% compatibility interval (CI) for the difference of means is reported*"}
library(flextable)
...
colnames <- c("","Measurement","Difference (Multi-Single)","3.5 %CI", "96.5 %CI")
flextable(indcallsummary)
...
```{r breadboardschem, echo=FALSE, fig.cap="\\label{fig:breadboardschem} Top: The experimental realisation of the audio-video synchronisation signal splitting. The components can easily be assembled onto a hobby breadboard, and are easily portable. Here the breadboard is pasted on the inside of a lunch box lid, allowing easy and safe transport of the breadboard and the Raspberry Pi in the box itself. Bottom: The circuit plan of the synchronisation signal splitter"}
