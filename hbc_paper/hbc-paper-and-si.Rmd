...
The audio and video feeds were synchronised using the method described in Laurijssen et
al. 2018. In short, ON-OFF signals with variable durations between 0.08-0.5 s
were generated by a portable computer (Raspberry Pi 3) and recorded on the audio
soundcard and used to drive the blinking of an LED that was recorded by the two cameras.
(See Supplementary Information (SI) \@ref(avmatching) for signal generation script, electronic circuit and
associated notes).
...
```{r fig1composite,echo=FALSE, out.width="100%",fig.cap='Individual and windowed call analysis examples. A-B: 150ms audio snippets from a single bat recording (A), and a multi-bat recording (B). The vertical lines indicate the start and stop of the 50 ms windows. C-D: Example of a manually selected CF-FM call, with its spectrogram (C) and waveform (D) representation. The call was automatically segmented into its initial frequency-modulated (iFM), central constant-frequency (CF) and terminal frequency-modulated (tFM) components based on frequency change over time , using the *itsfm* package. The itsfm package allows accurate segmentation into call components  under challenging recording conditions.'}
...
```{r incalldata, echo=FALSE, tab.cap="Difference between means and range of values for multi and single bat call parameters. The 93% compatibility interval (CI) for the difference of means is reported."}
library(flextable)

indcallsummary <- read.csv('../combined_analysis/all_ind_summary.csv')
indcallsummary <- indcallsummary[,2:ncol(indcallsummary)]
indcallsummary$single.range <- apply(singlebat.ranges,1,function(X){paste(as.character(X[1]),as.character(X[2]),sep='-')})
indcallsummary$multi.range <- apply(multibat.ranges,1,function(X){paste(as.character(X[1]),as.character(X[2]),sep='-')})
indcallsummary$lowerupper <- paste('(',as.character(indcallsummary$lower),',',as.character(indcallsummary$upper),')',sep='')

indcallsum.part <- indcallsummary[,c('component','parameter','lowerupper','single.range','multi.range')]


colnames <- c("Component","Measurement","Difference,(3.5,96.5)%CI", "Single bat range",'Multi bat range')

indtable <- flextable(indcallsum.part)
indtable <- set_header_labels(indtable, component=colnames[1], parameter=colnames[2], 
                              lowerupper=colnames[3], single.range=colnames[4],
                              multi.range=colnames[5])
indtable <- width(indtable, width = 1.25)
indtable
