...
1. Change the tables + any mentions of differences/numbers in the paper based on the results. 
...
We ran linear mixed-models (LMMs) with group size as the predictor and recording hour as random intercept for all parameters.  Model coefficients were estimated using a Bayesian approach with non-informative priors, and the 93% compatibility intervals were used in reporting.
...
```{r incalldata, echo=FALSE, tab.cap="Difference between multi and single bat call parameters. The 93% compatibility interval (CI) for the difference of means is reported"}
library(flextable)
